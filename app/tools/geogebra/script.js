export function tool(path) {
  ...
  /**
   * Deletes all active point and text (comment/label) objects from the
   * GeoGebra applet. Images and other object types are never touched.
   *
   * Hidden/disabled points are preserved. Text objects are deleted
   * regardless of their visibility flag, since GeoGebra reports
   * algebra-view-only text as "not visible" even when active.
   *
   * @returns {{
   *   success: boolean,
   *   deletedCount?: number,
   ...
   * }}
   */
  function deleteAllGeoGebraPoints() {
    // Make sure the GeoGebra applet is available.
    if (typeof ggbApplet === "undefined") {
      return {
        success: false,
        error: "GeoGebra applet was not found on this page.",
    ...
    let allNames;
    try {
      // Get every object currently present in the construction.
      allNames = ggbApplet.getAllObjectNames();
    } catch (e) {
      return {
        success: false,
        error: "Could not read objects from GeoGebra.",
      };
    }
    ...
    for (const name of allNames) {
      try {
        ...
        // Only points and text (comment/label) objects are eligible.
        // Images and everything else are left alone.
        if (type !== "point" && type !== "text") {
          continue;
        }

        // Hidden/disabled points are preserved. Text objects skip this
        // check, since they routinely report getVisible() === false
        // even when active.
        if (type === "point" && !ggbApplet.getVisible(name)) {
          continue;
        }

        // Skip undefined/inactive objects.
        if (
          typeof ggbApplet.isDefined === "function" &&
          !ggbApplet.isDefined(name)
        ) {
        ...
        // Ignore an object if GeoGebra does not allow it to be deleted.
      }
    }
