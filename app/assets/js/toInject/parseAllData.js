/**
 * Parses all data from the AIUB portal and returns a structured JSON object.
...
function getRoutine(){
  ...
  document
    .querySelectorAll(".StudentCourseList .panel.panel-primary")
    .forEach((panel) => {
      const body = panel.querySelector(".panel-body");

      const rawCourseLine = body.childNodes[0].textContent
        .trim()
        .match(/^(\d+)\s*-\s*(.*?)\s*\[(\w+)\]$/);
      ...
      const sectionStatus =
        body.querySelectorAll("label.label-info")[0]?.textContent.trim() || "";
      const status =
        body.querySelectorAll("label.label-success")[0]?.textContent.trim() ||
        "";
      const resultText =
        [...body.querySelectorAll("div")]
          .find((div) => div.textContent.includes("Result"))
          ?.innerText.trim()
          .replace("Result :", "")
          .trim() || "";
 ...
 */
 async function getCompletedCourseList(url) {
  let html = "";
  ...
  const tables =
    parser
      ...
      .querySelectorAll(".grade-report table") || [];
    ...
    for (let j = 1; j < rows.length; j++) {
      ...
      if (cells.length > 2) {
        ...
        if (grade.includes("[ - ]")) {
          ...
          if (
            currentYear === year &&
            currentSemester + 1 === semesterMap[semester]
          )
            continue;
        ...
        if (!grade) continue;
          ...
          courseList.push([courseCode, courseName, "Retake"]);
        else courseList.push([courseCode, courseName, ""]);
      }
    }
  }
  const nextSemster =
    currentSemester + 1 > 2 ? semesterMap[0] : semesterMap[currentSemester + 1];
  ...
  try {
    const firstTable = parser
      .parseFromString(html, "text/html")
      .querySelector(".grade-report table");
    const lastCellText = firstTable
      ?.querySelector("tr:nth-child(1) td:nth-child(6)")
      ?.innerText?.trim();
    const match = lastCellText?.includes("BSc")
      ? lastCellText?.match(/BSc(.*?),/)[1]
      : lastCellText.split(",")[0];
    craditCompleted = parseInt(
      firstTable
        ?.querySelector("tr:nth-child(3) td:nth-child(3)")
        ?.innerText?.trim(),
    );
